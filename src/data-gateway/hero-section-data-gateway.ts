...
export class HeroSectionDataGateway implements IHeroSectionDataGateway {
    ...
    async update(id: string, data: heroSectionDetailInput): Promise<heroSectionDetail> {
        const fileName = HERO_SECTION_FILE_ENV;

        const currentHeroes = await this.fetch();

        const indexToUpdate = currentHeroes.findIndex((currentHero) => currentHero.id === id);
    ...
    async delete(id:string): Promise<any> {
        ...
        const indexToDelete = currentHeroes.findIndex((currentHero) => currentHero.id === id);
